...
namespace IBI.<%= Name %>.Application.Utils.UI
{
    ...
    public static class BootstrapAutocompleteHelper
    {
        ...
        /// <returns></returns>
        public static IHtmlContent AutoCompleteFor<TModel, TProperty>(this IHtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression,
                                                                        ...
                                                                        string dataUrl, bool custom = false, string parentId = "", string typeName = "", string selectJavascriptFunction = "")
        {
            ...
            return new HtmlString(string.Format("{0} {1}", autoCompleteTextBox, autoCompleteScript));
        }
        ...
        /// <param name="hiddenFieldValue">The initial value of the hidden field</param>
        /// <returns></returns>
        public static IHtmlContent AutoCompleteFor<TModel, TProperty>(this IHtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, string textBoxId, string textBoxValue, string hiddenFieldId, string hiddenFieldValue)
        {
            var values = new Dictionary<string, object>();
            values.Add("id", textBoxId);
            values.Add("value", textBoxValue);
            var textBox = Utils.GetString(BootstrapTextBoxHelper.BootstrapTextBoxFor(htmlHelper, expression, values));
            var hidden = string.Format("<input type=\"hidden\" id=\"{0}\" name=\"{0}\" value=\"{1}\" />", hiddenFieldId, hiddenFieldValue);
            return new HtmlString(string.Format("{0}{1}", textBox, hidden));
        }
        ...
        /// <param name="selectJavascriptFunction">The name of the javascript function to run after a user has selects an search result</param>
        /// <returns></returns>
        public static IHtmlContent AutoCompleteScript(this IHtmlHelper helper, string textBoxId, string hiddenFieldId, string dataUrl, bool custom = false, string parentId = "", string typeName = "", string selectJavascriptFunction = "", int miscId = 0)
        {
